 ...
 * @author brasa
 */
public class InterfVodaService {

    private final EntityManagerFactory emf;

    public InterfVodaService() {
        emf = Persistence.createEntityManagerFactory("NetVisionKraljevoPU");
    }

    public List<Db.Interfvoda> getInterface() {

        EntityManager em = emf.createEntityManager();
        Query q = em.createQuery("SELECT i FROM Db.Interfvoda i ORDER BY i.br ASC");

        List<Db.Interfvoda> result = q.getResultList();
        em.close();
    ...
    public Db.Interfvoda getInterfaceWhere(int id) {
        EntityManager em = emf.createEntityManager();
        Query q = em.createQuery("SELECT i FROM Interfvoda i WHERE i.id = :id");
        q.setParameter("id", id);

        Db.Interfvoda result = (Db.Interfvoda) q.getSingleResult();
        em.close();
    ...
    private void updateNameComponent(int id, String komp, int rbr) {
        EntityManager em = emf.createEntityManager();
        Db.Interfvoda receptura = em.find(Db.Interfvoda.class, id);
    ...
    public void addNewRow(int brRed) {
        Db.Interfvoda interf = new Db.Interfvoda();
